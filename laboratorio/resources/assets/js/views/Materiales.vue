    ...
    <base-modal :active='active' @close='active=!active'>
      <template slot='modal-content'>
        <form class='columns is-multiline is-mobile m-0 p-t-sm p-l-md p-r-md p-b-md' @submit.prevent='submit' @keydown.enter.prevent='submit'>
          <p class='column is-12 has-text-weight-bold has-text-centered is-size-5 m-none'>Nuevo material</p>
...
import axios from 'axios'
  ...
  export default {
    ...
    methods: {
      getElements () {
        axios.get(`http://127.0.0.1:8000/api/Elements/materiales`)
        .then(response => {
      ...
      deleteElement () {
        axios.delete(`http://127.0.0.1:8000/api/Elements/materiales/${this.element.id}`)
        .then(response => {
      ...
      setDeleteModal (x) {
        this.initializeElement()
        axios.get(`http://127.0.0.1:8000/api/Elements/materiales/${x}`)
        .then(response => {
      ...
      search (name) {
        axios.post('http://127.0.0.1:8000/api/Elements/search/material', {
          search: name
        })
        .then(response => {
